...
const SignUp = () => {
  ...
  const handleSubmit = async (event) => {
    event.preventDefault();
    setIsSigningUp(true); // Start the signup process, show the loader
    ...
    try {
      ...
      setIsSigningUp(false); // Stop the signup process, hide the loader
    }
  };
  ...
  return (
    <div className="signup-form">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <br />
        {isSigningUp ? <Loader /> : <button type="submit">Sign Up</button>}
        <button type="button" onClick={redirectToLoginPage}>
          Go to Login
        </button>
      </form>
